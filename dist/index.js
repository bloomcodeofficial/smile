...
(() => {
  ...
  var player = () => {
    ...
    const audioCover = document.querySelector(".player_wrapper")?.querySelector(".player_audio-only-bg");
    modalContainer?.addEventListener("click", (e) => {
      if (e.currentTarget !== modalContainer) {
        modal?.classList.toggle("is-active");
      }
    });
    const audioPlyr = new import_plyr.default("audio", {});
    const modalPlyr = import_plyr.default.setup("#player-modal", {
      controls: [
        "play-large",
    ...
    const workPlayers = import_plyr.default.setup("#player-work-gallery", {
      controls: [
    ...
    workPlayersEl.forEach((player2, i) => {
      if (workPlayersData[i].dataset.category !== "Voices") {
        player2.addEventListener("mouseover", () => {
          workPlayers[i].play();
